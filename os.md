# {Note} 最进着迷于多任务不能自拔, 但是由于太浮躁, 太长的文章看不下去, 于是乎就想这样把文章一个字一个字打下来, 学习理解的同时也练习了打字
...
- 在任务中的 OSTimeDly();延时函数中, 我们需要进行人物调度. 当前任务延时了, 肯定就要换一个别的任务来运行了呀.
...
#define OS_GET_HIGH_RDY() {\
    for(char OSNextTaskPrio = 0; (OSNextTaskPrio < OS_TASKS) && (!(OSRdyTbl & (0x01 << OSNextTaskPrio))); OSNextTaskPrio++);\
